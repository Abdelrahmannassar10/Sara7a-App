...
import cloudinary from "../../utilies/cloud/cloudinary.config.js";
import fs from "fs";
export const deleteUser = async (req, res, next) => {
    if(req.user.profilePicture.public_id) {
        await cloudinary.api.delete_resources_by_prefix(
            `saraha_App/users/${req.user._id}`
        );
        await cloudinary.api.delete_folder(`saraha_App/users/${req.user._id}`);
    }
...
export const uploadProfilePictureCloud = async (req, res, next) => {
    ...
    if(!req.user.profilePicture.public_id) {
        await cloudinary.uploader.upload(file.path, {
            folder: `saraha_App/users/${user._id}/profilePicture`
        });
    }
